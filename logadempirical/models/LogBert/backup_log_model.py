...
#        x = batch["sequential"]
        if isinstance(batch, (list, tuple)):
           print("WARNING: batch is list, collate_fn failed!")
           batch = {
               "sequential": batch[0],
    ...
    # --- temporary debug fix ---
        ...
        # --- if x is a list of variable-length sequences ---
        if isinstance(x, list):
            # convert each sequence to a tensor
            sequences = [torch.tensor(seq, dtype=torch.long, device=device) for seq in x]
            # pad sequences to same length
            x = pad_sequence(sequences, batch_first=True, padding_value=0)  # shape: (batch, max_seq_len)
        ...
        # Make sure time_info and segment_info are padded the same way
        if time_info is not None:
            if isinstance(time_info, list):
                time_info = pad_sequence([torch.tensor(t, dtype=torch.float, device=device) for t in time_info],
                                     batch_first=True, padding_value=0)

        segment_info = torch.zeros_like(x)  # if not provided, create zeros
        x = self.bert(x, time_info=segment_info)
        x = self.mask_lm(x)
        logits = self.fc2(x)
